...
func TestConstraintsMatchVersions(t *testing.T) {
	...
	// Model-definition error checks (matchversions only allowed on scalar
	// attrs, never nested under array/map/ifvalues/wildcard) moved to
	// registry.TestConstraintsMatchVersionsModelErrors (pure model
...
// TestConstraintsGroupTypeModelSerialization verifies GET /model correctly
// serializes a valid model, including a Group-type "constraints"
// definition. The model-definition ERROR checks (bad constraint
// key/path/enum/equals combos, formerly part of this function) moved to
// registry.TestConstraintsGroupTypeErrors (pure model verification, no
// DB needed).
func TestConstraintsGroupTypeModelSerialization(t *testing.T) {
	reg := NewRegistry("TestConstraintsGroupTypeModelSerialization")
	defer PassDeleteReg(t, reg)
